# -*- coding: utf-8 -*-
# Copyright (C) 2021 github.com/XenixYT
#
...
if __name__ == '__main__':
    ...
    try:
        github_data = request_client.get("").json()
...
╚═╝    ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝╚═╝╚═╝  ╚═╝    ╚═╝  ╚═╝╚══════╝╚══════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚══════╝
{colours['white']}
> Made with {colours['main_colour']}<3{colours['white']} by {colours['main_colour']}.xenix1{colours['white']}
> Discord: {colours['main_colour']}https://discord.gg/NW39fY3pER{colours['white']}
> Don't forget to {colours['green']}Vouch{colours['white']} <3!
> Version: {colours['main_colour']}{app_version}""")
        
    try: ctypes.windll.kernel32.SetConsoleTitleW(f"Xenix Account Restore | .gg/NW39fY3pER")
    except: pass

    if app_latest_ver != app_version:
        print()
        c.warn(f"You are running an outdated version!: {app_latest_ver}")
...
{colours['light_blue']}>{colours['white']} Choice ({colours['main_colour']}int{colours['white']}) """, end="")
        ...
        if choice == 1:
            ...
            c.info('For additional help, join the Discord Server: https://discord.gg/NW39fY3pER')
            c.info("If it's invalid, go to https://icy.sell.app and click the Discord icon.")
